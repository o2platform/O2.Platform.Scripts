...
namespace test
{
	//based on the code sample from http://www.developerfusion.com/code/4693/using-the-credential-management-api/
	//							and http://www.developerfusion.com/code/4693/using-the-credential-management-api/2/
	public sealed class API_SecureCredentials
    {
		
		public static void test()
        {
            askUserForPassword(null); 
        }
        
        public static bool askUserForPassword(string defaultName)
        {
        	var applicationName = "O2 Passwords";
        	Func<string,string,bool> areCredentialsOK = 
        		(string name, string password)=>{	
									        		"Logging in with:  {0}:{1}".info(name, password);  
									            	// return true if the authentication succeeds
									            	return password == "1234";	 								            	
									        	};
			askUserForPassword(defaultName, applicationName, areCredentialsOK);							        	
        	return false;	
        }
        
        public static bool askUserForPassword(string name, string applicationName, Func<string,string,bool> credentialsOkCallback)
        {
            bool value = false;
            try
            {
                CredentialsDialog dialog = new CredentialsDialog(applicationName);
                if (name != null) 
                	dialog.AlwaysDisplay = true; // prevent an infinite loop
                if (dialog.Show(name) == DialogResult.OK)
                {
                    if (credentialsOkCallback(dialog.Name, dialog.Password))
                    {
                        value = true;
                    ...
                    else
                    {
                        try
                        {
                        	"in dialog.Confirm(false)".error();
                            dialog.Confirm(false);
                        }
                        catch (ApplicationException applicationException)
                        {
                         	applicationException.log();
                        }
                        value = askUserForPassword(dialog.Name, applicationName,credentialsOkCallback); // need to find a way to display 'Logon unsuccessful'
                    }
                }
            }
            catch (ApplicationException applicationException)
